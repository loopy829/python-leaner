...
class DateCountdown:
    def __init__(self, parent):
        ...
        tk.Label(parent, text="目标日期（格式：YYYY-MM-DD）", font=("Arial", 12)).pack()
        self.date_entry = tk.Entry(parent, font=("Arial", 14), width=20, justify="center")
        self.date_entry.insert(0, "2025-12-01")
        self.date_entry.pack(pady=5)

        tk.Button(parent, text="开始倒计时", command=self.start_countdown).pack(pady=5)
        self.label = tk.Label(parent, text="📅 倒计时尚未开始", font=("Arial", 14))
    ...
    def start_countdown(self):
        try:
            ...
            messagebox.showerror("日期格式错误", "请输入正确的格式：YYYY-MM-DD")
    ...
    def update_countdown(self):
        while self.running:
            ...
            if delta > 0:
                self.label.config(text=f"⏳ 距离目标还有：{delta} 天")
            elif delta == 0:
                self.label.config(text="🎉 今天就是目标日！")
                self.running = False
            else:
                self.label.config(text="✅ 已超过目标日")
                self.running = False
            time.sleep(60)  # 每分钟更新一次

# ====== 界面构建 ======
root = tk.Tk()
root.title("效率工具箱：音乐推荐 + 番茄钟 + 日期倒计时")
...
# 日期倒计时区域
tk.Label(root, text="📆 日期倒计时", font=("Arial", 14, "bold")).pack(pady=5)
frame3 = tk.Frame(root)
frame3.pack(pady=5)
DateCountdown(frame3)
